...
class VyvoTTSHQQInference:
    """TTS inference engine using HQQ-optimized Transformers backend."""
    ...
    def __init__(
        ...
        """Initialize the TTS inference engine with HQQ optimization.
        ...
        Args:
            ...
            compute_dtype: Computation dtype for the model
            nbits: Number of bits for HQQ quantization
            group_size: Group size for HQQ quantization
        """
        # Load configuration
    ...
    def _initialize_models(self, model_name: str):
        ...
        AutoHQQHFModel.quantize_model(
            self.model,
            quant_config=BaseQuantizeConfig(nbits=self.nbits, group_size=self.group_size, axis=1),
            compute_dtype=self.compute_dtype,
            device=self.device
        )

        # Prepare for optimized inference
        prepare_for_inference(self.model, backend="gemlite")

        # Note: patch_model_for_compiled_runtime is disabled because LFM2 model
        # contains dynamic control flow that is not compatible with torch.compile
    ...
    def generate(self, text: str, voice: Optional[str] = None,
        ...
        generated_ids, generation_time = self._generate_text(
            input_ids, attention_mask, do_sample, max_new_tokens,
...
def main():
    """Example usage of VyvoTTSHQQInference."""
